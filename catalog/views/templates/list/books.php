...
<div class="abgne_tab">
  ...
  <strong class="date">
			...
			<p class="textM"><?php echo  strtoupper(date('M', strtotime($searchbook[$i]['write_time']))); ?></p>
			<p class="textD">/<?php echo date('j', strtotime($searchbook[$i]['write_time'])); ?></p>	
		  </strong>  
            <div class="content">
                <div class="inner">
                    <h3 class="title"><?php echo $searchbook[$i]['title']; ?></h3>
                    <p class="description"><?php echo mb_substr(strip_tags(html_entity_decode($searchbook[$i]['content'])),0,72,"UTF-8"); ?>...</p>
                    <a class="read-more" href="../article/view/<?php echo $searchbook[$i]['article_id']; ?>">Continue Reading →</a>
                </div>
            </div>  
            </div>
    </li>            
